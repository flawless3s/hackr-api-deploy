...
from fastapi import FastAPI, HTTPException, Header, Depends, UploadFile, Request
from fastapi.middleware.cors import CORSMiddleware
from typing import List
from pydantic import BaseModel
from dotenv import load_dotenv
...
app = FastAPI(
    title="RAG Q&A API",
    description="A memory-optimized RAG API using LlamaIndex + Groq",
    version="1.0.0"
)
...
# Load LLM + Embedding once at startup
def configure_llama_index():
    groq_api_key = os.getenv("GROQ_API_KEY")
    if not groq_api_key:
...
service_context = configure_llama_index()
...
@app.get("/")
def root():
    return {"status": "running", "model": "llama3.1-8b", "provider": "Groq"}
...
@app.post("/api/v1/hackrx/run", response_model=HackathonResponse)
async def run_submission(request: Request, _: None = Depends(validate_token)):
    try:
        content_type = request.headers.get("content-type", "")
        questions, file, document_url = [], None, None
        ...
        if "application/json" in content_type:
            ...
            if questions_raw:
                ...
                questions = [v for k, v in form.items() if k.startswith("question")]
            document_url = form.get("document_url")
            file = form.get("file")
        else:
        ...
        if file is not None:
            ...
            raise HTTPException(status_code=400, detail="No valid document provided")
